...
<body class="hold-transition layout-top-nav">
				...
				<div class="container-fluid px-5">
					<div class="row">
						<div class="col-md">
							...
							<!-- <button type="button" class="btn btn-sm btn-warning float-right" data-toggle="modal" data-target="#rekon_modal">
								Rekonsialisasi
							</button> -->
						</div>
					</div>
						...
						<div class="col-md-12">
							<div class="collapse" id="li_anggota">
								<div class="card card-primary card-outline">
									...
									<div class="card-body">
										<table class="table table-bordered table-hover display nowrap" id="tbl_example" style="width: 100%;">
											<thead>
												<tr>
													<th>#</th>
													<th>Nomor Kontrak</th>
											...
											<tbody>
												<?php foreach ($anggota as $key => $val) : ?>
													<tr>
														...
														<td class="text-center"><?= tgl_indo(substr($val['tgl_pencairan'], 0, 10)) ?></td>
			...
			<div class="modal-content">
				...
				<form id="fm_upload">
					<div class="modal-body">
						...
						<input type="hidden" class="form-control" name="noloan_kop" value="<?= $koperasi['rek_pembayaran'] ?>">
	...
	<!-- <div class="modal fade" id="rekon_modal" data-backdrop="static" data-keyboard="false" tabindex="-1">
		<div class="modal-dialog modal-lg">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title" id="rekon_modalLabel">Rekonsialisasi anggota koperasi</h5>
					<button type="button" class="close" data-dismiss="modal" aria-label="Close">
						<span aria-hidden="true">&times;</span>
					</button>
				</div>
				<form id="fm_upload">
					<div class="modal-body">
						<label>1. Download file template</label>
						<p>
							Download template file rekonsialisasi anggota koperasi. File ini memiliki kolom header sesuai data yang diperlukan untuk import rekonsialisasi anggota koperasi.<br>
							<a href="<?= site_url('sales/koperasi-channeling/template/end-user') ?>"><i class="fa fa-fw fa-file-alt"></i> Download File Template</a>
						</p>
						<hr>
						<label>2. Input data template</label>
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					<form id="fm_anggota" autocomplete="off">
						...
						<input type="hidden" class="form-control" id="noloan_kop" name="noloan_kop" value="<?= $this->uri->segment(3) ?>">
						<div class="form-group row">
						...
						<div class="form-group row">
							<label for="os_pokok" class="col-sm-3 col-form-label">OS Pokok</label>
							<div class="col-sm-3">
				...
				<div class="modal-footer">
					<button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
					<button type="button" onclick="save_form()" class="btn btn-primary">Submit</button>
				</div>
			</div>
		</div>
	</div>
		...
		function save_form() {
			...
			$.ajax({
				...
				beforeSend: function() {
					...
					$('.btn-primary').prop('disabled', true).html('<i class="fa fa-fw fa-pulse fa-spinner"></i> Loading');
				},
		...
		function sunting(id) {
			...
			$.ajax({
				...
				success: function(res) {
					...
					$('#tunggakan').val(new Intl.NumberFormat().format(res.tunggakan));
				}
			});
		}
